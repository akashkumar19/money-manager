...
@Component({
  ...
  standalone: true,
  imports: [ButtonModule, DropdownModule, CalendarModule, ReactiveFormsModule, InputNumberModule, InputTextModule, CardModule],
...
export class AddTransactionComponent {
  ...
  onSubmit() {
    ...
    if(this.id) {
      this.httpService.put(`${this.transactionUrl}${this.id}/`, this.transaction.value).subscribe({
        complete: () => {
      ...
      this.httpService.post(this.transactionUrl, this.transaction.value).subscribe((response: any) => {console.log(response.results, "------------"),
        this.router.navigate(["transaction"])
      })
    }
  }
